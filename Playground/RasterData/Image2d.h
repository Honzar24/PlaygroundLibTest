...
public:
	...
	static Image2d<T> CreateWithSingleValue(int w, int h, const T * value, ColorSpace::PixelFormat pf);
	...
	Image2d<T> CreateWithBorder(int w, int h) const;
	...
	std::array<size_t, 8> GetNeighborsIndicesFromPosition(size_t index) const noexcept;
	std::array<size_t, 8> GetNeighborsIndicesFromPosition(int x, int y) const noexcept;
	std::array<T *, 8> GetNeighborsPixelStartsFromPosition(size_t index) noexcept;
	std::array<T *, 8> GetNeighborsPixelStartsFromPosition(int x, int y) noexcept;
	std::array<const T *, 8> GetNeighborsPixelStartsFromPosition(size_t index) const noexcept;
	std::array<const T *, 8> GetNeighborsPixelStartsFromPosition(int x, int y) const noexcept;

	void FindMinMax(size_t channelIndex, T & min, T & max) const;
	T CalcAvgValue(size_t channelIndex) const;
	...
	void SetSubImage(int x, int y, const Image2d<T>& img);
