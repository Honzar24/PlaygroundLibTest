...
PNGLoader::DecompressedImage PNGLoader::DecompressWithLodePNG(uint8_t * mem, size_t memSize)
{
	...
	pngState.decoder.color_convert = 0; //keep input data channels count
...
void PNGLoader::LibPNGReadPalette(DecompressedImage & d)
{
	...
	if (retTRNS == PNG_INFO_tRNS)
	{
		...
		for (int i = 0; i < num_palette; i++)
		{
            ...
            d.palette.push_back(std::move(rgba));
			*/
			d.palette.emplace_back(palette[i].red, palette[i].green, palette[i].blue,
				(i < num_trans) ? trans[i] : 255);

			d.grayScalePallete &= d.palette.back().IsGrayScale();
		}
	}
	else
	{
